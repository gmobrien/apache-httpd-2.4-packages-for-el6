...
Patch100: apr-util-1.5.3-httpd24-soname.patch
BuildRoot: %{_tmppath}/%{name}-%{version}-%{release}-buildroot
BuildRequires: autoconf, %{?scl:%scl_prefix}apr-devel >= 1.3.0
...
Requires: %{?scl:%scl_prefix}apr-util%{?_isa} = %{version}-%{release}, %{?scl:%scl_prefix}apr-devel%{?_isa}, pkgconfig
...
Requires: %{?scl:%scl_prefix}apr-util%{?_isa} = %{version}-%{release}
...
%configure --with-apr=%{_prefix} \
        ...
        --with-sqlite3 --with-pgsql --with-mysql --without-freetds --with-odbc \
        --with-berkeley-db=%{_prefix}/include:%{_libdir} \
        --without-sqlite2 \
...
* Tue Sep 17 2013 Jan Kaluza <jorton@redhat.com> - 1.5.2-5
- set "-release httpd24" when building as SCL

* Fri Apr 19 2013 Jan Kaluza <jkaluza@redhat.com> - 1.5.2-4
